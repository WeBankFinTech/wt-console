...
const isHexColor = str => {
  const u = '[0-9a-zA-Z]'
  return [new RegExp(`^#${u}{3}$`), new RegExp(`^#${u}{4}$`), new RegExp(`^#${u}{6}$`), new RegExp(`^#${u}{8}$`)].some(
    item => item.test(str)
  )
}
const isRGBColor = str => {
  const d = '\\s*\\d+(\\.\\d+)?\\s*'
  return [new RegExp(`^rgb\\(${d},${d},${d}\\)$`), new RegExp(`^rgba\\(${d},${d},${d},${d}\\)$`)].some(item =>
    item.test(str)
  )
}
...
const isHSLColor = str => {
  const d = '\\s*\\d+(\\.\\d+)?%?\\s*'
  return [new RegExp(`^hsl\\(${d},${d},${d}\\)$`), new RegExp(`^hsla\\(${d},${d},${d},${d}\\)$`)].some(item =>
    item.test(str)
  )
}

const isColor = str => {
  return namedColors.indexOf(str) > -1 || isHexColor(str) || isRGBColor(str) || isHSLColor(str)
}
